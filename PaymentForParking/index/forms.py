...
class paidparkingForm(forms.ModelForm):
  class Meta:
      ...
      widgets = {
          ...
          'startofvalidityperiod': forms.DateTimeInput(attrs={"type": "datetime-local","class": "form form-control form-control-lg"}),
          'expirationdate': forms.TextInput(attrs={"class": "form form-control form-control-lg", "readonly": 0}),
      }
  def clean_telephone(self):
        telephone = self.cleaned_data['telephone']
        r = re.compile('^\+7|8\D*\d{3}\D*\d{3}\D*\d{2}\D*\d{2}')
        if r.search(telephone):
            pass
        else:
...
class paidseasonticketsForm(forms.ModelForm):
   class Meta:
              ...
              fields = ['nametickets','carnumber', 'price', 'email', 'telephone','startofvalidityperiod','expirationdate']
              widgets = {
                  ...
                  'startofvalidityperiod': forms.DateTimeInput(attrs={"type": "datetime-local", "class": "form form-control form-control-lg"}),
                  'expirationdate': forms.TextInput(attrs={"class": "form-control form-control-lg form", "readonly": 0}),
              }
